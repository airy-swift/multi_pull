...
class ActionWidget extends StatelessWidget {
  ActionWidget({@required this.icon, this.label, this.action, this.onRefresh})
...
class MultiPull extends StatefulWidget {
  ...
  final List<ActionWidget> actionWidget;
...
class MultiPullState extends State<MultiPull>
  ...
  void initState() {
    ...
    _horizonPositionFactor =
        _horizonPositionController.drive(_kDragHorizonSizeFactorLimitTween);
    ...
    if (notification is ScrollStartNotification &&
      ...
      setState(() {
        ...
        _indicator = Container(
          ...
          child: Row(
            children:
                widget.actionWidget.map((e) => Expanded(child: e)).toList(),
          ),
        );
      });
      return false;
  ...
  bool _start(AxisDirection direction) {
    ...
    width = _key.currentContext.size.width * _widgetScale;
    final spaceWidth = 1 / (widget.actionWidget.length + 1);
    clampList =
        List.generate(widget.actionWidget.length, (i) => (i + 1) * spaceWidth);
    return true;
  }
  ...
  void _checkDragOffset(DragUpdateDetails details) {
    ...
    if (_mode == _RefreshIndicatorMode.armed) {
      _horizonPositionController.value =
          ((details.globalPosition.dx * _widgetScale) / width)
              .clamp(0.15, 0.85);
      print(_horizonPositionController.value);
    }
  ...
  int _clamp(double value) {
    ...
    final _min = _clampList.reduce(min);
  ...
  void _show() {
    ...
    final selectedIndex = _clamp(_horizonPositionController.value);

    if (widget.actionWidget[selectedIndex].action != null) {
      widget.actionWidget[selectedIndex].action();

      completer.complete();
      _dismiss(_RefreshIndicatorMode.done);
    } else if (widget.actionWidget[selectedIndex].onRefresh != null) {
      ...
      final Future<void> refreshResult =
          widget.actionWidget[selectedIndex].onRefresh();
      assert(() {
                  ...
                  child: AnimatedBuilder(
                      ...
                      builder: (context, child) {
                        return Transform.translate(
                          child: Opacity(
                            opacity: _mode == _RefreshIndicatorMode.refresh || _mode == _RefreshIndicatorMode.done
                                ? 0.0
                                : 0.3,
                            child: CircleAvatar(
                          ...
                          offset: Offset(
                            _horizonPositionController.value * width -
                                (width / 2),
                            0,
                          ),
                        );
