...
/// Defines the layout and behavior of a [ExBar].
///
/// See also:
///
///  * [ExBar]
...
enum ExBarType {
  /// The [ExBar]'s [ExBarItem]s have fixed width.
  fixed,

  /// The location and size of the [ExBar] [ExBarItem]s
...
/// where it is provided as the [Scaffold.ExBar] argument.
///
/// The bottom navigation bar's [type] changes how its [items] are displayed.
/// If not specified, then it's automatically set to
/// [ExBarType.fixed] when there are less than four items, and
/// [ExBarType.shifting] otherwise.
///
///  * [ExBarType.fixed], the default when there are less than
...
///  * [ExBarType.shifting], the default when there are four
...
///    [ExBarItem.backgroundColor] of the selected item. In this
///    case it's assumed that each item will have a different background color
///    and that background color will contrast well with white.
///
/// {@tool dartpad --template=stateful_widget_material}
/// This example shows a [ExBar] as it is used within a [Scaffold]
/// widget. The [ExBar] has three [ExBarItem]
...
///   return Scaffold(
///     appBar: AppBar(
///       title: const Text('ExBar Sample'),
///     ),
///     body: Center(
///       child: _widgetOptions.elementAt(_selectedIndex),
///     ),
///     ExBar: ExBar(
///       items: const <ExBarItem>[
///         ExBarItem(
...
class ExBar extends StatefulWidget {
  /// Creates a bottom navigation bar which is typically used as a
  /// [Scaffold]'s [Scaffold.ExBar] argument.
  ///
  /// The length of [items] must be at least two and each item's icon and title
  /// must not be null.
  ///
  /// If [type] is null then [ExBarType.fixed] is used when there
  /// are two or three [items], [ExBarType.shifting] otherwise.
  ///
  ...
  /// [ExBarType.fixed] and `false` if [type] is
  /// [ExBarType.shifting].
  ExBar({
    Key key,
    @required this.items,
    ...
    this.mouseCursor,
        ...
        assert(
          items.every((ExBarItem item) => item.label != null),
  ...
  final List<ExBarItem> items;

  /// Called when one of the [items] is tapped.
  ///
  /// The stateful widget that creates the bottom navigation bar needs to keep
  /// track of the index of the selected [ExBarItem] and call
  /// `setState` to rebuild the bottom navigation bar with the new [currentIndex].
  final ValueChanged<int> onTap;

  /// The index into [items] for the current active [ExBarItem].
  final int currentIndex;

  /// The z-coordinate of this [ExBar].
  ///
  ...
  /// Defines the layout and behavior of a [ExBar].
  ///
  /// See documentation for [ExBarType] for information on the
  /// meaning of different types.
  final ExBarType type;
  ...
  /// The color of the [ExBar] itself.
  ///
  /// If [type] is [ExBarType.shifting] and the
  /// [items] have [ExBarItem.backgroundColor] set, the [items]'
  /// backgroundColor will splash and overwrite this color.
  final Color backgroundColor;

  /// The size of all of the [ExBarItem] icons.
  ///
  /// See [ExBarItem.icon] for more information.
  final double iconSize;

  /// The color of the selected [ExBarItem.icon] and
  /// [ExBarItem.title].
  ///
  /// If null then the [ThemeData.primaryColor] is used.
  final Color selectedItemColor;

  /// The color of the unselected [ExBarItem.icon] and
  /// [ExBarItem.title]s.
  ///
  ...
  /// The [TextStyle] of the [ExBarItem] labels when they are
  /// selected.
  final TextStyle selectedLabelStyle;

  /// The [TextStyle] of the [ExBarItem] labels when they are not
  /// selected.
  final TextStyle unselectedLabelStyle;

  /// The font size of the [ExBarItem] labels when they are selected.
  ///
  ...
  /// The font size of the [ExBarItem] labels when they are not
  /// selected.
  ///
  ...
  /// Whether the labels are shown for the selected [ExBarItem].
  final bool showUnselectedLabels;

  /// Whether the labels are shown for the unselected [ExBarItem]s.
...
class _ExBarState extends State<ExBar> with TickerProviderStateMixin {
  ...
  // [ExBarType.fixed] for 3 or fewer items, and
  // [ExBarType.shifting] is used for 4+ items.
  ExBarType get _effectiveType {
    return widget.type ??
        BottomNavigationBarTheme.of(context).type ??
        (widget.items.length <= 3 ? ExBarType.fixed : ExBarType.shifting);
  }

  // Computes the default value for the [showUnselected] parameter.
  //
  // Unselected labels are shown by default for [ExBarType.fixed],
  // and hidden by default for [ExBarType.shifting].
  ...
  @override
  void didUpdateWidget(ExBar oldWidget) {
